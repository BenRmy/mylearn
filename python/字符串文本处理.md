...
此模块提供用于比较序列的类和函数。它可以用于比较文件，并可以产生各种格式的不同信息，包括 HTML 和上下文以及统一格式的差异点。要比较目录和文件，可使用filecmp模块。

class difflib.SequenceMatcher：用于比较任何类型的序列对，只要序列元素是可清除的。

class difflib.Differ：用来比较文本里的行，并且产生可阅读的差异化结果。

class difflib.HtmlDiff：用来创建一个html表格（或者包含html表格的文件）用来展示文件差异。他既可以进行全文本展示，也可以只展示上下文不同。

+ 构造函数__init__(tabsize=8, wrapcolumn=None, linejunk=None, charjunk=IS_CHARACTER_JUNK)：tabsize表示制表符代表的空格个数，默认为8；wrapcolumn,可选参数，用来设置多少个字符时自动换行，默认None,为None时表示不自动换行；linejunk 和 charjunk，可选参数，在ndiff()中使用。
+ 公共方法make_file(fromlines, tolines [, fromdesc][, todesc][, context][, numlines])：用来生成一个包含表格的html文件，其内容是用来展示差异。fromlines和tolines,用于比较的内容，格式为字符串组成的列表；fromdesc和todesc，可选参数，对应的fromlines,tolines的差异化文件的标题，默认为空字符串；context和numlines，可选参数，context为True时，只显示差异的上下文，为false，显示全文；numlines默认为5，当context为True时，控制展示上下文的行数，当context为false时,控制不同差异的高亮之间移动时“next”的开始位置（如果设置为0，当移动顶端时，超链接会丢失引用地址）
+ 公共方法make_table(fromlines, tolines [, fromdesc][, todesc][, context][, numlines])：这个方法和make_file用法一样，唯一的区别在于它只生成了一个html表格字符串。

difflib.context_diff(a, b[, fromfile][, tofile][, fromfiledate][, tofiledate][, n][, lineterm])：用来比较a,b都是字符串列表。返回一个格式化文本的差异。

difflib.get_close_matches(word, possibilities[, n][, cutoff])：返回一个列表中最相似的word（典型的应用是字符串列表）。

difflib.ndiff(a, b[, linejunk][, charjunk])：比较a和b,返回差异。linejunk和charjunk都是用来匹配的方法。

difflib.restore(sequence, which)：返回1或2的差异对Differ.compare()或者ndiff()的结果进行处理，根据参数which,返回片段1或者片段2的差异化结果实例

difflib.unified_diff(a, b[, fromfile][, tofile][, fromfiledate][, tofiledate][, n][, lineterm])：用来比较a,b都是字符串列表。返回一个统一的格式化文本的差异

difflib.IS_LINE_JUNK(line)：如果有需要忽略的行，则返回为true,如果这个行内全为空格或者只有’#’,则将这行忽略掉

difflib.IS_CHARACTER_JUNK(ch)：如果有需要忽略的字符，则返回为true,如果这个字符为空格,则将这行忽略掉

## csv--CSV 文件读写

CSV (Comma Separated Vaules) 格式是电子表格和数据库中最常见的输入、输出文件格式。尽管分隔符会变化，但此类文件的大致格式是相似的，所以编写一个单独的模块以高效处理此类数据，将程序员从读写数据的繁琐细节中解放出来是有可能的。csv模块实现了以CSV格式读取和写入表格数据的类。csv模块中的 reader 类和 writer 类可用于读写序列化的数据。也可使用 DictReader 类和 DictWriter 类以字典的形式读写数据。

常用函数：

+ csv.reader(csvfile, dialect='excel', **fmtparams)：返回一个读取器对象，它将迭代给定csvfile文件中的行。csvfile可以是任何支持迭代器协议的对象，每次__next__()调用其方法时都返回一个字符串--文件对象和列表对象都是合适的。如果csvfile是文件对象，则应添加参数newline=''。其他的可选参数，用于定义其他功能。
...
+ csv.writer(csvfile, dialect='excel', **fmtparams)：返回一个writer对象，负责将用户的数据转换为给定的类文件对象上的分隔字符串。csvfile可以是带有write()方法的任何对象。如果csvfile是文件对象，则应使用newline=''打开它。其他的可选参数，用于定义其他功能。为了使与实现DB API的模块接口尽可能简单，将值None写为空字符串。虽然这不是可逆转换，但它可以更容易地将SQL NULL数据值转储到CSV文件，而无需预处理cursor.fetch调用返回的数据。所有其他非字符串数据str()在写入之前都会进行字符串化。
...
+ class csv.DictReader(f, fieldnames=None, restkey=None, restval=None, dialect='excel', *args, **kwds)：创建一个像常规阅读器一样操作的对象，但将每行中的信息映射到OrderedDict中，其键由可选的fieldnames参数给出。如果省略fieldnames，则文件f的第一行中的值将用作字段名。无论字段名如何确定，有序字典都保留其原始顺序。如果一行包含的字段多于字段名，则将剩余数据放入一个列表中，并使用restkey指定的字段名（默认为None）进行存储。如果非空行的字段少于字段名，则缺少的值将填入None。所有其他可选或关键字参数都传递给基础reader实例。
...
+ class csv.DictWriter(f, fieldnames, restval='', extrasaction='raise', dialect='excel', *args, **kwds)：创建一个像常规编写器一样操作的对象，但将字典映射到输出行。注意：与DictReader类不同，类DictWriter的fieldnames参数不是可选的。
...
格式参数：在创建reader或writer对象时，程序员可以指定类的字符串或子类Dialect作为dialect参数。除了dialect参数之外，程序员还可以指定单独的格式化参数，这些参数与为Dialect类定义的属性具有相同的名称。

+ Dialect.delimiter：用于分隔字段的单字符字符串。它默认为','。
+ Dialect.doublequote：控制如何引用字段中出现的quotechar实例。当为False时，escapechar作为quotechar前缀。它默认为True。
+ Dialect.escapechar：由writer使用的一个字符串转换为逃避分隔符，如果QUOTE_NONE与quotechar被设置为False。默认为None，禁用转义。
+ Dialect.lineterminator：用于终止由writer生成的行的字符串。它默认为'\r\n'。
+ Dialect.quotechar：一个单字符的字符串，用于引用包含特殊字符的字段，例如delimiter或quotechar，或者换行符。它默认为'"'。
+ Dialect.quoting：控制何时应由writer生成引号并由reader识别。它可以采用任何QUOTE_*常量并默认为QUOTE_MINIMAL。
+ Dialect.skipinitialspace：若为True，忽略分隔符后面的空格。默认是False。
+ Dialect.strict：若为True，在错误的CSV输入上引发异常Error。默认是False。

## configparser---配置文件解析器

此模块实现了基本配置语言的类，该语言提供类似于Microsoft Windows INI文件中的结构。可以使用它来编写可由最终用户轻松定制的Python程序。模块shlex支持创建类似Unix shell语言，可用作应用程序配置文件的备用格式。json模块实现了JavaScript语法的子集，也可用于此目的。
...
如上所述，API非常简单。唯一注意的是DEFAULT，其为所有其他部分提供默认值的部分。注意，部分键不区分大小写并以小写形式存储。
配置解析器不会猜测配置文件中值的数据类型，始终将它们作为字符串存储在内部。这意味着如果需要其他数据类型，需要自己转换。由于此任务非常常见，因此配置解析器提供了一系列方便的getter方法来处理整数，浮点数和布尔值。
有趣的是，因为简单地传递值bool()将没有好处，因为bool('False')仍然是True。这就是配置解析器getboolean()提供的原因。此方法不区分大小写，并从'yes'/'no'，'on'/'off'， 'true'/'false'和'1'/'0'中识别布尔值。除getboolean()之外，配置解析器还提供等效的getint()和 getfloat()方法。与字典一样，还可以使用section节的get()方法来提供后备值。也可以注册自己的转换器或自定义提供的转换器。
